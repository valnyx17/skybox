...
function keymap.set(mode, lhs, rhs, opts)
    require("legendary").keymap({
        ...
        description = (opts and opts.desc) or "",
    })
end
...
function keymap.create_mapper(mode)
...
keymap.setup = function()
    ...
    keymap.setn("<C-s>", "<cmd>w<CR>", { desc = "write file" })
    ...
    keymap.setn("<C-M-h>", ":vert resize +2<CR>", { desc = "resize split left" })
    keymap.setn("<C-M-j>", ":resize -2<CR>", { desc = "resize split down" })
    keymap.setn("<C-M-k>", ":resize +2<CR>", { desc = "resize split up" })
    keymap.setn("<C-M-l>", ":vert resize -2<CR>", { desc = "resize split right" })
    ...
    keymap.setv("x", '"_x')
    keymap.setv("X", '"_X')
    keymap.setv("j", "v:count == 0 ? 'gj' : 'j'", { expr = true })
    keymap.setv("k", "v:count == 0 ? 'gk' : 'k'", { expr = true })
