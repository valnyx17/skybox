...
_G.sky = {
    utils = require("sky.utils"),
    ui = require("sky.ui.globals"),
    lsp = require("sky.lsp"),
}
...
sky.get_path_root = function(path)
    if path == "" then
        return
    end
...
sky.get_git_remote_name = function(root)
    if root == nil then
        return
    end
...
sky.set_git_branch = function(root)
...
sky.get_git_branch = function(root)
    if root == nil then
        return
    end
    ...
    return sky.set_git_branch(root)
end

sky.is_nonprog_ft = function()
    return sky.nonprog_modes[vim.bo.filetype] ~= nil
end

--------------------------------------------------
-- LSP
--------------------------------------------------
sky.diagnostics_available = function()
...
---@param ns_id integer?
---@return table
sky.get_hl_hex = function(opts, ns_id)
...
sky.group_number = function(num, sep)
    if num < 999 then
        return tostring(num)
    else
        num = tostring(num)
